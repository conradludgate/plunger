...
use crate::pin_list::list::NodeAcquired;
use crate::pin_list::list::NodeLinked;
use crate::pin_list::list::NodeProtected;
use crate::pin_list::list::NodeReleased;
use crate::pin_list::list::NodeShared;
use crate::pin_list::list::OptionNodeShared;
use crate::pin_list::util::debug_unreachable;
use crate::pin_list::util::unwrap_unchecked;
...
impl<'node, T: ?Sized + Types> InitializedNode<'node, T> {
    ...
    /// Panics if the given list is a different one than this node was registered with.
    #[must_use]
    pub fn acquired<'list>(&self, list: &'list PinList<T>) -> Option<&'list T::Acquired> {
        assert_eq!(self.inner().list_id, list.id, "incorrect `PinList`");

        // SAFETY: We have shared access to both `self` and our `PinList`, as guaranteed above.
        match unsafe { &*self.shared().protected.get() } {
            NodeProtected::Released(..) | NodeProtected::Linked(..) => None,
            NodeProtected::Acquired(acquired) => Some(&acquired.data),
        }
    }
    ...
    /// Panics if the given list is a different one than this node was registered with.
    #[must_use]
    pub fn acquired_mut<'list>(
        &self,
        list: &'list mut PinList<T>,
    ) -> Option<&'list mut T::Acquired> {
        assert_eq!(self.inner().list_id, list.id, "incorrect `PinList`");
        ...
        match unsafe { &*self.shared().protected.get() } {
            ...
            NodeProtected::Acquired(..) => {
                ...
                Some(match unsafe { &mut *self.shared().protected.get() } {
                    NodeProtected::Acquired(acquired) => &mut acquired.data,
                    NodeProtected::Released(..) | NodeProtected::Linked(..) => unsafe {
                        debug_unreachable!()
                    },
                })
            }
    ...
    pub fn unlink(
        ...
        let linked = match unsafe { &mut *self.shared().protected.get() } {
            ...
            NodeProtected::Acquired(..) | NodeProtected::Released(..) => return Err(self),
        };
    ...
    pub fn take_removed(
        ...
        match unsafe { &*self.shared().protected.get() } {
            NodeProtected::Released(..) => {}
            NodeProtected::Acquired(..) | NodeProtected::Linked(..) => return Err(self),
        }
    ...
    T::Released: Debug,
{
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Released(removed) => f.debug_tuple("NodeData::Released").field(removed).finish(),
        }
    }
}
